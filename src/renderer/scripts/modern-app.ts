...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function formatPseudocode(): void {
        ...
        try {
            ...
            out(`❌ Format failed: ${(error as Error).message}`, 'error');
        } finally {
    ...
    function restoreTabStatusAndConsole() {
        ...
        // Initialize outputs count separately
        if (typeof (consoleStats as any).outputs === 'undefined') {
            (consoleStats as any).outputs = 0;
        }
    ...
    function applyUIVisibilitySettings(settings: any) {
        ...
        if (stopButton) {
            // Stop button should be hidden by default, only shown during execution
    ...
    function execute(code: string): void {
        if (!code || !code.trim()) { out('⚠️ No code to execute','warning'); return; }
        try {
        // Set execution state
        isExecuting = true;
        (window as any).isExecuting = isExecuting;
        executionStopped = false;
            ...
            runnerWorker.onmessage = (ev) => {
                ...
                try {
                    ...
                    out('🔧 Handler error: ' + (err as Error).message, 'error'); 
                    cleanupWorker(); 
                }
            };
    ...
    function showEditorForTab(tab: HTMLElement): void {
    ...
    function showConsoleForTab(tab: HTMLElement): void {
        // First switch to the tab
        switchToTab(tab);
    ...
    if (saveButton) {
        saveButton.addEventListener('click', async () => {
            ...
            try {
                ...
                out(`💾 Save failed: ${(error as Error).message}`, 'error');
            }
        });
    }
