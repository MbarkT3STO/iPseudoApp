...
(function(){
    class ErrorSystem {
        analyze(errorLike, code) {
            ...
            const parsed = { line: 0, column: 0, message: err.message || String(err), name: err.name || 'Error', stack };
            ...
            // Provide a small context extractor
            parsed.getContext = (fullCode, contextLines=2) => {
        ...
        formatForConsole(info, context=null) {
        ...
        applyEditorMarkers(editor, info) {
            if (!editor || !info || !info.line) return;
            try {
                const range = new monaco.Range(info.line, Math.max(1, info.column||1), info.line, Math.max(1, info.column||1));
                const dec = [{ range, options: { isWholeLine: true, className: 'severity-high', glyphMarginClassName: 'error-glyph', hoverMessage: { value: info.message } } }];
            ...
            } catch(e) { console.error('applyEditorMarkers failed', e); }
        }
    }
