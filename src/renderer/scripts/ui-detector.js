...
class UIDetector {
    ...
    detectCurrentUI() {
        ...
        const filename = path.split('/').pop() || href.split('/').pop();
        
        console.log('UI Detection - Path:', path, 'Filename:', filename);
        
        // Check for modern UI files
        if (path.includes('modern-ui') || href.includes('modern-ui') || 
            filename === 'index-modern.html' || filename === 'modern-app.html' ||
            filename === 'modern-workspace.html' || filename === 'modern-ui.html') {
            console.log('Detected: Modern UI');
            return 'modern';
        }
        
        // Check for classic UI files
        if (filename === 'index.html' || filename === 'index-old.html' ||
            path.includes('src/renderer') || path.includes('dist/renderer') || 
            href.includes('src/renderer') || href.includes('dist/renderer')) {
            console.log('Detected: Classic UI');
            return 'classic';
        }
        
        console.log('Detected: Unknown UI');
        return 'unknown';
    }

    // Initialize UI detection and redirection
    init() {
        console.log('UI Detector Init - Preferred:', this.preferredUI, 'Current:', this.currentUI);
        
        // Check if we've already redirected recently to prevent infinite loops
        const lastRedirect = localStorage.getItem('lastUIRedirect');
        const now = Date.now();
        if (lastRedirect && (now - parseInt(lastRedirect)) < 5000) {
            console.log('Skipping redirect - too recent');
            return;
        }
        ...
        setTimeout(() => {
            ...
            if (this.preferredUI !== this.currentUI && this.currentUI !== 'unknown') {
                console.log('Redirecting to preferred UI:', this.preferredUI);
                localStorage.setItem('lastUIRedirect', now.toString());
                this.redirectToPreferredUI();
            } else {
                console.log('No redirect needed - already in preferred UI');
            }
        }, 100);
    }
    ...
    setPreferredUI(ui) {
        try {
            ...
            localStorage.removeItem('lastUIRedirect'); // Clear redirect timestamp
            this.preferredUI = ui;
