    ...
    class ErrorSystem {
        analyze(errorLike, code) {
            ...
            const parsed = {
                ...
                name: err.name || 'Error',
                stack,
                getContext: (fullCode, contextLines = 2) => {
                    if (!fullCode)
                        return null;
            ...
            if (!parsed.line || isNaN(parsed.line))
                parsed.line = 0;
            if (!parsed.column || isNaN(parsed.column))
                parsed.column = 0;
            return parsed;
        }
        formatForConsole(info, context = null) {
            ...
            if (info.line)
                out += ` (line ${info.line}${info.column ? ',' + info.column : ''})`;
            if (context) {
                out += '\n--- Context ---\n';
                if (context.before)
                    out += context.before + '\n';
        ...
        applyEditorMarkers(editor, info) {
            if (!editor || !info || !info.line)
                return;
            try {
                const range = new window.monaco.Range(info.line, Math.max(1, info.column || 1), info.line, Math.max(1, info.column || 1));
                const dec = [{
                        range,
                        options: {
                            isWholeLine: true,
                ...
                if (this._decorations)
                    this._decorations = editor.deltaDecorations(this._decorations, dec);
                else
...
//# sourceMappingURL=errorSystem.js.map